...
public class ButtonComponent extends UIComponent {
    ...
    public ButtonComponent(final MenuComponent menuOwner, final int iteration, final Setting setting) {
    ...
    public void mouseDown(int posX, int posY, int button) {
        if (this.hovered) {
            this.method.execute(this.menuOwner.module);
        }
    }

    /**
     * Renders this component to the screen.
     * @param guiAlpha the alpha value of the GUI.
     */
    public void draw(final float guiAlpha) {
        int offset = 3;

        Color colour = new Color(regularColour.getRed(), regularColour.getGreen(), regularColour.getBlue(), Math.round(255 * guiAlpha));
        ...
        colour = new Color(this.colour.getRed(), this.colour.getGreen(), this.colour.getBlue(), Math.round(255 * guiAlpha));
        Simple.drawRoundedRect(this.posX + 10, this.posY + offset, this.width, this.height, colour.getRGB(), 5, Simple.Rect.ALL);

        colour = new Color(255, 255, 255);
        Instance.getInstance().getFontManager().getFontRenderer("Montserrat 14").drawStringWithShadow(this.title, this.posX + 15, this.posY + 4 + offset, colour.getRGB());
    }
}
